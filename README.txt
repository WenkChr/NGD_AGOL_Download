...
`requirements.txt` file, and can be installed with `pip3 install -r requirements.txt`.
 ...
 All of the scripts expect certain environment variables to be set so that it knows where to find the data and how to parse 
 it. If you don't want to set environment variables manually, you can create a `environments.env` file with the following 
 contents (adjust for your environment):
...
For ease of use, the file can be created in the same directory as 'automate_download.py', `detect_changes.py`, 
and 'automate_upload.py'.
...
Change detect is done by running the `detect_changes.py` script. 
...
### Step 3 - Upload FeatureClass to AGOL

The FeatureClass is added back to the AGOL environment to allow NGD editors to load it into their desktop environment 
as a reference layer. Layer is also shared with the 'NGD' AGOL group for them to access and settings are changed to
allow for export into format of choice